...
- **What's done is transform-level only; 1.12.2 mods do not fully run in-game yet.** The class-move table and ctor bridge are unit-tested bytecode building blocks, and until snapshot.7 a 1.12.2 mod wasn't even *scanned* by modern Forge/NeoForge (item 1 below fixed that), so the rest still doesn't run in-game. The in-game prerequisites, in order:
