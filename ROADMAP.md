...
- **Chunk force-loading: `ServerChunkCache.addRegionTicket` (1.18-era chunk mods, e.g. Chunky).** Pinned precisely (this session, from intermediary `method_17297`): a 1.16-1.20 mod calls `ServerChunkCache.addRegionTicket(TicketType, ChunkPos, int, T value)` (4 args), but 26.1 renamed it to `addTicketWithRadius(TicketType, ChunkPos, int)` (3 args) **and dropped the value arg**, so the call hits `NoSuchMethodError` and worlds crash on load. A plain rename can't fix it (different arity); it needs an **arg-dropping rewrite**: pop the trailing `value` off the stack, then call `addTicketWithRadius`. Retromod already has the stack-manip primitive (`neutralizeCall` pops args), so the work is a small opt-in `registerArgDropRedirect` redirect type plus a 26.1-gated registration. Caveat: it must fire at the right point relative to the intermediary->Mojang rename pass (key on the names the method-redirect visitor actually sees), and Chunky additionally mixins `ThreadedAnvilChunkStorage`, so the adapter unblocks the crash but full Chunky compatibility also needs that mixin checked. Needs in-game verification.
