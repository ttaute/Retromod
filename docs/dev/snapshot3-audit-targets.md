...
| `ScreenEvents$AfterRender` / `HudRenderCallback` | 7 / 6 | event SAMs; `HudRenderCallback` is gone from the host entirely — find successor before classifying |
...
| `ConventionalItemTags` | 5 | `tag/convention/v1`→`v2`; tag fields, but some were renamed in the v1→v2 move — a blanket redirect risks `NoSuchFieldError`, so it needs per-field entries |
...
Sources: the **official Fabric API 26.1 migration map** (139 class entries —
...
- **NeoForge `LivingDamageEvent$Post.getNewDamage`**: rename to `getInflictedDamage` landed in 26.1.2.**70**-beta; the current host line (.60) still has `getNewDamage` — do not redirect until hosts move.
...
- **Datapack worldgen is a hard limitation**: mods shipping 1.20.x datapack worldgen
  (biomes/features/carvers JSON) fail 26.1 registry load entirely — blocks ALL world
