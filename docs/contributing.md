...
If that produces `target/retromod-1.1.0-rc.1.jar`, you're good. Java 25 is required. See [Troubleshooting]({{ '/troubleshooting' | relative_url }}) if the build complains about class file versions.
...
```bash
cp target/retromod-1.1.0-rc.1.jar \
  ~/Library/Application\ Support/minecraft/mods/retromod-1.1.0-rc.1+26.1.jar
```
