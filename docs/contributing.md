...
If that produces `target/retromod-1.0.0-beta.1.jar`, you're good. Java 25 is required — see [Troubleshooting]({{ '/troubleshooting' | relative_url }}) if the build complains about class file versions.
...
```bash
cp target/retromod-1.0.0-beta.1.jar \
  ~/Library/Application\ Support/minecraft/mods/retromod-1.0.0-beta.1+26.1.jar
```
