...
Old Forge mods (anything built with ForgeGradle's `reobfJar` task — basically every Forge mod for MC < 1.20.5) reference Minecraft members by Forge's intermediate "SRG" names rather than Mojang's official names. So instead of `Blocks.STONE`, the bytecode actually says `Blocks.f_50069_`. Forge's runtime classloader used to remap these to the real names at class-load time.
...
`KIND` is `FIELD` or `METHOD`. Lines starting with `#` are comments. Blank lines are ignored. Owner class is intentionally not recorded — SRG names are unique across the entire MC namespace, so a single mapping is sufficient regardless of which class the field or method lives on.
...
Use a real tab character between columns. Many editors show whitespace mode if you're not sure — copy an existing row and replace the names if in doubt.
...
- **Don't mass-import without verifying.** A PR that dumps 5,000 untested SRG names from MCP CSV is harder to review than 50 entries you've actually exercised against a real mod.
