...
Retromod takes over: it loads a dictionary of SRG → Mojang member names and applies them via the same `ClassRemapper` pipeline that handles intermediary → Mojang for Fabric mods. The dictionary ships with full 1.20.1-era coverage (about 53,000 entries, generated by joining Forge's MCPConfig data with Mojang's official mappings), so contributions now focus on corrections and on SRG names from other MC versions that real crash logs surface.
...
The shipped dictionary is already generated by exactly this join (see the provenance header at the top of the tsv), so bulk hand-importing is no longer needed. Hand-add entries only for corrections, or for SRG names from a different MC version that a real crash log surfaced.

## Adding the entry

Open `src/main/resources/retromod/srg-to-mojang.tsv`. The file has two sections, FIELDS and METHODS; add your row to the matching section (rows are sorted by SRG id, so slot it in numerically).
...
```
Loaded SRG → Mojang mappings: 23582 methods, 29891 fields (53473 parsed, 0 skipped)
```
...
Full 1.20.1-era coverage (about 53,000 entries) ships with Retromod today. Contributed corrections and version-drift entries make the next person's mod load that much further.
