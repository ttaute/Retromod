...
- **Status**: **1.2.0** ("the general update") is the current stable release (Forge 26.2 hosts, worldgen on 26.x). Keep backups of your mod JARs.
...
1. Put `retromod-1.2.0+<your-mc>.jar` in `mods/`.
...
Fabric checks mod versions *before* Retromod runs, so old mods can't go straight into `mods/`. The `retromod-input/` folder is the inbox Retromod converts from. (One-shot via CLI: `java -jar retromod-cli.jar prepare ~/.minecraft --aot`.)
